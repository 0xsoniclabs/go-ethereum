...
var upstreamEvmFields = map[string]reflect.Type{
	...
	"arena":       reflect.TypeOf((*stackArena)(nil)),
}
...
var sonicHookSites = []hookSite{
	...
	},
	{
		...
		wiring: "called twice from stateTransition.settleGas (core/state_transition.go): once " +
			"on the pre-Prague path, before the EIP-3529 refund cap is computed, and once " +
			"post-Prague, after the EIP-7623 floor-gas adjustment",
		dirs:  []string{coreDir},
		count: 2,
	...
	},
	{
		...
		wiring: "consulted twice - a nil check and a dereference - in " +
			"stateTransition.maxTxGas (core/state_transition.go), which supplies the cap to " +
			"both the Osaka-gated EIP-7825 check in preCheck and the Amsterdam regular-gas " +
			"split in initRuntimeGasBudget",
		dirs:  []string{coreDir},
		count: 2,
	...
	},
	{
		...
		wiring: "called three times in EVM.initNewContract (core/vm/evm.go), once per " +
			"fork branch: EIP-4762, Amsterdam and the pre-Amsterdam default. Upstream " +
			"checks the limit separately per branch because its position relative to the " +
			"gas charge differs; a branch bypassing this call would let Sonic's custom " +
			"code-size limit apply on some paths only",
		dirs:  []string{coreDir, coreVmDir},
		count: 3,
	...
	},
	{
		...
		wiring: "called three times: from stateTransition.preCheck (core/state_transition.go) " +
	...
	// EmitLogsForBurnAccounts used to be censused here as upstream v1.17.2
	// behavior Sonic depends on. Upstream removed EIP-7708 burn logs entirely in
	// EIP-8246 (#35219), so there is no longer a call site to pin.
}
...
// to the interceptor to the CallContextInterceptor method(s) it must delegate to.
// EVM.create is the shared creation frame reached from Create, Create2 and the
// CREATE/CREATE2 opcodes alike, and picks its interceptor method by opcode.
// Adding a further entry point means adding one row here and one row in
...
func TestSonicHookRegistry_EveryCallEntryPointStillDispatches(t *testing.T) {
		...
		for _, decl := range file.Decls {
			...
			ast.Inspect(function, func(node ast.Node) bool {
				if method, ok := interceptorDispatch(node); ok {
					...
					dispatches[name] = append(dispatches[name], method)
				}
	...
	for _, entryPoint := range slices.Sorted(maps.Keys(expectedCallInterceptorDispatches)) {
		...
		if !found {
			t.Errorf("EVM.%s no longer dispatches to the call interceptor; restore\n"+
				...
				entryPoint, want[0], entryPoint, registryMaintenanceHint)
			continue
		}
		if !slices.Equal(slices.Sorted(slices.Values(got)), want) {
			t.Errorf("EVM.%s dispatches to CallContextInterceptor.%v, expected .%v. %s",
	...
	for _, entryPoint := range slices.Sorted(maps.Keys(dispatches)) {
		if _, expected := expectedCallInterceptorDispatches[entryPoint]; !expected {
			t.Errorf("EVM.%s dispatches to CallContextInterceptor.%v but is not in the "+
...
var sonicHookTests = map[string][]string{
	...
	"EVM.CallInterceptor": {
		...
		"TestCallInterceptor_CreateOpcodesDispatchToTheInterceptor",
		"TestSonicHookRegistry_EveryCallEntryPointStillDispatches",
	},
	// The two entries below pin upstream behavior that Sonic depends on rather
	// than Sonic modifications of upstream.
