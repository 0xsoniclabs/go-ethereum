...
var (
	...
	statePrecompileGasLeft = uint64(4_711)
)
...
func TestStatePrecompiles_UnregisteredAddressKeepsVanillaBehavior(t *testing.T) {
	...
	for name, registered := range precompiles {
		t.Run(name, func(t *testing.T) {
			...
			ret, gasLeft, err := evm.Call(common.Address{0x01}, target, []byte("call-data"), 100_000, uint256.NewInt(0))
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			if len(ret) != 0 {
			...
			if gasLeft != 100_000 {
				t.Errorf("expected the supplied gas to be returned unchanged, got %d", gasLeft)
			}
