...
// Stack
// Sonic: a stack handed to Step outlives the run it is stepped through, so it gets
// its own arena instead of claiming a region of an EVM's.
func NewStack() *Stack {
	arena := &stackArena{data: make([]uint256.Int, initialStackSize)}
	return arena.stack()
}
...
type CallContextInterceptor interface {
	...
	Call(env *EVM, me common.Address, addr common.Address, data []byte, gas GasBudget, value *uint256.Int) ([]byte, GasBudget, error)
	// CallCode takes another contracts code and execute within our own context
	CallCode(env *EVM, me common.Address, addr common.Address, data []byte, gas GasBudget, value *uint256.Int) ([]byte, GasBudget, error)
	// DelegateCall is same as CallCode except sender and value is propagated from parent to child scope
	DelegateCall(env *EVM, me common.Address, addr common.Address, data []byte, gas GasBudget) ([]byte, GasBudget, error)
	// Create creates a new contract
	Create(env *EVM, me common.Address, data []byte, gas GasBudget, value *uint256.Int) ([]byte, common.Address, GasBudget, error)
	// Create2 creates a new contract with a deterministic address
	Create2(env *EVM, me common.Address, code []byte, gas GasBudget, value *uint256.Int, salt *uint256.Int) ([]byte, common.Address, GasBudget, error)
	// StaticCall executes a contract in read-only mode
	StaticCall(env *EVM, me common.Address, addr common.Address, input []byte, gas GasBudget) ([]byte, GasBudget, error)
}
