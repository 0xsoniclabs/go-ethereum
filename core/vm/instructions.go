...
func opCreate(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	...
	res, addr, result, suberr := evm.create(scope.Contract.Address(), input, child, &value, contractAddr, CREATE, nil)
...
func opCreate2(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	...
	res, addr, result, suberr := evm.create(scope.Contract.Address(), input, child, &endowment, contractAddr, CREATE2, &salt)
