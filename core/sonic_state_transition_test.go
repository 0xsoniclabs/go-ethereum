...
func TestStateTransition_ExcessGasIsChargedAfterTheFloorGasAdjustment(t *testing.T) {
	...
	intrinsicGas, err := IntrinsicGas(data, nil, nil, false, true, true, true)
	if err != nil {
...
// TestStateTransition_ANilGasPoolIsTolerated pins the Sonic modification in
// ApplyMessage that substitutes a message-sized gas pool for a nil one.
// Upstream dereferences the pool unconditionally and panics.
...
// TestStateTransition_BurnAccountLogsAreEmittedUnderAmsterdamRules pins the
// Sonic-threaded vm.StateDB.EmitLogsForBurnAccounts call site at the end of
// stateTransition.execute.
func TestStateTransition_BurnAccountLogsAreEmittedUnderAmsterdamRules(t *testing.T) {
	tests := map[string]struct {
		fork      fork
		wantCalls int
	}{
		"pre-Amsterdam": {fork: osaka, wantCalls: 0},
		"Amsterdam":     {fork: amsterdam, wantCalls: 1},
	}
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			db := &dummyStateDB{}
			if _, err := runTestTransactionOnStateDB(msg, vm.Config{}, chainConfigUpTo(test.fork), db); err != nil {
				t.Fatalf("Error running transaction: %v", err)
			}
			if db.emitLogsForBurnAccountsCalls != test.wantCalls {
				t.Fatalf("expected %d call(s) to StateDB.EmitLogsForBurnAccounts, got %d; "+
					"the Amsterdam-gated call site at the end of stateTransition.execute "+
					"is not effective", test.wantCalls, db.emitLogsForBurnAccountsCalls)
			}
		})
	}
}
...
func TestStateTransition_InsufficientFundsForTransferCheckCanBeDisabled(t *testing.T) {
	msg := &Message{
		From:     common.Address{12},
		Value:    big.NewInt(1), // - affordable for buyGas, rejected by the transfer guard
		GasLimit: 100_000,
	}
...
// hooks that are fork-gated - the two chargeExcessGas call sites, the
// Osaka-gated MaxTxGas cap and the Amsterdam-gated EmitLogsForBurnAccounts call
// - need this to pin every gated path.
...
type dummyStateDB struct {
	...
	emitLogsForBurnAccountsCalls int
}

func (db *dummyStateDB) EmitLogsForBurnAccounts() {
	db.emitLogsForBurnAccountsCalls++
}
